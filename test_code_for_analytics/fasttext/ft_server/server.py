...
@app.route("/neighborsel", methods=["POST"], endpoint="neighborsel")
def neighborsel():
    queries = json.loads(request.data)
    res = [post_nearest_neighbors(el) for el in queries]
...
@app.before_request
def before_request():
    g.ft_model = fasttext.load_model('/cc.ru.300.bin')
...
def post_nearest_neighbors(q, k, threshold):
    retrieval_method = g.ft_model.get_nearest_neighbors(q, k, threshold)

    if len(q.split(" ")) > 1:
        retrieval_method = g.ft_model.get_nearest_neighbors
